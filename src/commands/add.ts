...
import { addProfile, profileExists, isSSHKeyInUse } from '../config/config-manager';
...
export async function addCommand(): Promise<void> {
  ...
  if (existingGitConfig) {
    ...
    console.log(chalk.gray('  (You can use different values for this profile)\n'));
  }
  ...
  const prompts: any[] = [
    {
      ...
      validate: async (input: string) => {
        ...
        if (await profileExists(input.trim())) {
          return `Profile "${input.trim()}" already exists. Choose a different name.`;
        }
        return true;
      },
    },
